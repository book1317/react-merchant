const catImage =
  'https://cf.shopee.co.th/file/18d8e8ef2e7ecd66a2ddbe885a01df30_tn'

export const INVENTORIES = [
  {
    name: 'เป้สวยจัด สภาพใหม่มาก 98%',
    imageURL:
      'https://cf.shopee.co.th/file/90078a1bf6b41aa74e041ff0b92f6862_tn',
    price: 250.0,
    remaining: 10,
    status: 'PENDING',
  },
  {
    name: 'ผ้าลายการ์ตูนอะนิเมะสําหรับตกแต่งห้องนอนเด็ก',
    imageURL:
      'https://cf.shopee.co.th/file/18d8e8ef2e7ecd66a2ddbe885a01df30_tn',
    price: 151.0,
    remaining: 0,
    status: 'SOLD',
  },
  {
    name: '⚡️พร้อมส่ง⚡️ รองเท้าผ้าใบ รองเท้าแฟชั่น รองเท้าผ้าใบผู้หญิง',
    imageURL:
      'https://cf.shopee.co.th/file/bcb9021314691e77510ec49884297e26_tn',
    price: 100.0,
    remaining: 50,
    status: 'ACTIVE',
  },
  {
    name:
      '[ผ่อน0% 10 เดือน] Playstation 4 Pro PS4 Pro Used (สินค้ามือ2) สภาพใหม่ ประกันศูนย์ไทย พร้อมเกมส์ 5 เกมส์',
    imageURL:
      'https://cf.shopee.co.th/file/e3a9fa22b73b419917682935f7a60f40_tn',
    price: 7990.0,
    remaining: 10,
    status: 'PENDING',
  },
  {
    name: 'VIDVIEW รองเท้าแตะสวมเด็ก VV 2502 เบา ใส่สบาย สีสดใส (ไซส์ 11-3)',
    imageURL:
      'https://cf.shopee.co.th/file/bfe6515990836c2aa1979d5e26c8574b_tn',
    price: 100.0,
    remaining: 0,
    status: 'SOLD',
  },
  {
    name:
      '[+..••] สินค้าเตรียมจัดส่ง 25 - 28 ก พ | PS5 PLAYSTATION 5 CONSOLE (เกมส์ PlayStation 5™🎮)',
    imageURL:
      'https://cf.shopee.co.th/file/60508befa1fd5bfb3d0dbfcfe10823b7_tn',
    price: 28067.0,
    remaining: 0,
    status: 'SOLD',
  },
  {
    name:
      'Seoul.fluffy เสื้อยืด โอเวอร์ไซส์ รุ่นลายปัก my life สไตล์เอิร์ธโทน มินิมอล เนื้อผ้านิ่ม ใส่สบาย',
    imageURL:
      'https://cf.shopee.co.th/file/eb866227d2be0563780c8777d45d6d76_tn',
    price: 100.0,
    remaining: 0,
    status: 'SOLD',
  },
  {
    name:
      'Tulip หมอนอิง ลิเวอร์พูล Liverpool (ขนาด 48×48 ซม.) ลิขสิทธิ์แท้ VV008',
    imageURL:
      'https://cf.shopee.co.th/file/310ebc96654d1b1e7404599134b7ed85_tn',
    price: 550.0,
    remaining: 50,
    status: 'ACTIVE',
  },
  {
    name: 'ผ้าแขวนผนังลายการ์ตูนอนิเมะสําหรับตกแต่งห้อง',
    imageURL:
      'https://cf.shopee.co.th/file/e39ce9013ccc5559004ca494e3aa433a_tn',
    price: 158.0,
    remaining: 40,
    status: 'PENDING',
  },
  {
    name: 'เสื้อผ้าผู้ใหญ่',
    imageURL: catImage,
    price: 100.0,
    remaining: 2,
    status: 'ACTIVE',
  },
  {
    name: 'เสื้อผ้าผู้ใหญ่',
    imageURL: catImage,
    price: 100.0,
    remaining: 3,
    status: 'ACTIVE',
  },
]
